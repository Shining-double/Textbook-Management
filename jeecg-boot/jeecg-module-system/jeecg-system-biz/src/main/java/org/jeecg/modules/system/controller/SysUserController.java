...
public class SysUserController {
	...
	public Result<?> delete(@RequestParam(name="id",required=true) String id) {
        try {
            ...
            int delDepart = sysUserDepartService.getBaseMapper().delete(
                    new LambdaQueryWrapper<SysUserDepart>().eq(SysUserDepart::getUserId, id)
            );
            log.info("删除关联数据：用户-角色({}条)、用户-部门({}条)", delRole, delDepart);

            // 步骤2：核心修复！用LambdaUpdateWrapper替代LambdaQueryWrapper，支持set()方法
            SysUserMapper sysUserMapper = (SysUserMapper) sysUserService.getBaseMapper();
            // 构建更新条件：LambdaUpdateWrapper（用于更新），而非LambdaQueryWrapper（用于查询）
            int updateDelFlag = sysUserMapper.update(
                    new SysUser(), // 空对象，仅通过wrapper设置更新字段
                    new LambdaUpdateWrapper<SysUser>() // 关键：换成LambdaUpdateWrapper
                            .set(SysUser::getDelFlag, "1") // 现在能找到set方法了
                            .eq(SysUser::getId, id) // 更新条件：匹配ID
            );
            log.info("原生SQL更新del_flag：id={}，影响行数={}", id, updateDelFlag);

            if (updateDelFlag <= 0) {
                log.error("强制更新del_flag失败：ID={}（可能ID不存在/字段映射错误）", id);
                // 直接跳过del_flag，执行终极删除
                log.warn("跳过del_flag标记，直接物理删除");
                int delUser = sysUserMapper.delete(
                        new LambdaQueryWrapper<SysUser>().eq(SysUser::getId, id)
                );
                if (delUser <= 0) {
                    return Result.error("删除失败：ID不存在或字段映射错误！");
                }
                baseCommonService.addLog("直接物理删除用户：id=" + id, CommonConstant.LOG_TYPE_2, 3);
                return Result.ok("删除成功！数据库中已无该用户记录");
            }

            // 步骤3：调用deleteLogicDeleted删除（此时del_flag已=1）
            List<String> userIds = Collections.singletonList(id);
            int delUser = sysUserMapper.deleteLogicDeleted(userIds);
            if (delUser <= 0) {
                // 兜底：再次直接删除
                delUser = sysUserMapper.delete(new LambdaQueryWrapper<SysUser>().eq(SysUser::getId, id));
            }

            // 步骤4：验证+返回
            log.info("彻底删除用户成功：id={}，影响行数={}", id, delUser);
            baseCommonService.addLog("彻底删除用户：id=" + id, CommonConstant.LOG_TYPE_2, 3);
            return Result.ok("删除成功！数据库中已无该用户记录");
	...
	public Result<?> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
        try {
            ...
            // 1. 拆分并清洗ID（去空格、空值，避免无效ID）
            List<String> userIdList = Arrays.stream(ids.split(","))
                    .map(String::trim)
                    .filter(id -> !id.isEmpty())
                    .collect(Collectors.toList());

            if (userIdList.isEmpty()) {
                return Result.error("批量删除的用户ID不能为空！");
            }
            log.info("批量删除准备：待处理用户ID数量={}", userIdList.size());

            // 2. 步骤1：批量删除关联数据（对齐单个删除的delRole/delDepart逻辑）
            // 2.1 批量删除用户-角色关联（用getBaseMapper.delete，返回int，和单个删除一致）
            int delRole = sysUserRoleService.getBaseMapper().delete(
                    new LambdaQueryWrapper<SysUserRole>().in(SysUserRole::getUserId, userIdList)
            );
            // 2.2 批量删除用户-部门关联（有该表则保留，无则注释）
            int delDepart = sysUserDepartService.getBaseMapper().delete(
                    new LambdaQueryWrapper<SysUserDepart>().in(SysUserDepart::getUserId, userIdList)
            );
            log.info("批量删除关联数据：用户-角色({}条)、用户-部门({}条)", delRole, delDepart);

            // 3. 步骤2：核心！批量更新del_flag=1（对齐单个删除的updateDelFlag逻辑）
            SysUserMapper sysUserMapper = (SysUserMapper) sysUserService.getBaseMapper();
            int updateDelFlag = sysUserMapper.update(
                    new SysUser(), // 空对象，仅通过wrapper设置更新字段
                    new LambdaUpdateWrapper<SysUser>()
                            .set(SysUser::getDelFlag, "1") // 字符串1，匹配数据库类型
                            .in(SysUser::getId, userIdList) // 批量更新条件：ID in 列表
            );
            log.info("批量更新del_flag=1：待更新数量={}, 实际更新行数={}", userIdList.size(), updateDelFlag);

            if (updateDelFlag <= 0) {
                log.error("批量更新del_flag失败：无用户被标记为逻辑删除（可能ID不存在/字段映射错误）");
                // 直接跳过del_flag，执行终极批量物理删除
                log.warn("跳过del_flag标记，直接批量物理删除");
                int delUsers = sysUserMapper.delete(
                        new LambdaQueryWrapper<SysUser>().in(SysUser::getId, userIdList)
                );
                if (delUsers <= 0) {
                    return Result.error("批量删除失败：无用户记录被删除（ID不存在或字段映射错误）！");
                }
                baseCommonService.addLog("直接批量物理删除用户：ids=" + ids, CommonConstant.LOG_TYPE_2, 3);
                return Result.ok("批量删除成功！数据库中已无对应用户记录");
            }

            // 4. 步骤3：调用deleteLogicDeleted批量删除（对齐单个删除的deleteLogicDeleted逻辑）
            int delUser = sysUserMapper.deleteLogicDeleted(userIdList);
            log.info("调用deleteLogicDeleted批量删除：待删数量={}, 实际删除行数={}", userIdList.size(), delUser);

            if (delUser <= 0) {
                // 兜底：批量物理删除（对齐单个删除的兜底逻辑）
                log.warn("deleteLogicDeleted批量删除失败，执行兜底批量物理删除");
                delUser = sysUserMapper.delete(
                        new LambdaQueryWrapper<SysUser>().in(SysUser::getId, userIdList)
                );
            }

            // 5. 保留原有逻辑：获取用户名+触发工作流
            List<String> userNameList = sysUserService.userIdToUsername(userIdList);
            if (!userNameList.isEmpty()) {
                String joinedString = String.join(",", userNameList);
                log.info("批量删除用户触发工作流：用户名={}", joinedString);
            }

            // 6. 验证+返回（对齐单个删除的验证逻辑）
            log.info("批量删除用户成功：待删数量={}, 最终删除行数={}", userIdList.size(), delUser);
            baseCommonService.addLog("彻底批量删除用户：ids=" + ids, CommonConstant.LOG_TYPE_2, 3);
            return Result.ok("批量删除成功！数据库中已无对应用户记录");
