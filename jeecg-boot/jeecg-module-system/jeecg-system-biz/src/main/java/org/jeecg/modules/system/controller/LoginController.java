...
public class LoginController {
	...
	@Autowired
	private ISysUserRoleService sysUserRoleService; // 用户-角色关联表服务
	@Autowired
	private ISysRoleService sysRoleService; // 角色表服务
	...
	public Result<JSONObject> getUserInfo(HttpServletRequest request){
		...
		if(oConvertUtils.isNotEmpty(username)) {
			...
			JSONObject userInfoJson = (JSONObject) JSONObject.toJSON(sysUser); // 将SysUser转为JSON对象
			String roleCode = getRoleCodeByUserId(sysUser.getId()); // 查询角色编码
			userInfoJson.put("roleCode", roleCode); // 新增roleCode字段
			obj.put("userInfo", userInfoJson);
...
//			obj.put("userInfo",sysUser);
	...
	private Result<JSONObject> userInfo(SysUser sysUser, Result<JSONObject> result, HttpServletRequest request, String clientType) {
		...
		//3.设置登录用户信息
		JSONObject userInfoJson = (JSONObject) JSONObject.toJSON(sysUser);
		String roleCode = getRoleCodeByUserId(sysUser.getId());
		userInfoJson.put("roleCode", roleCode);
		obj.put("userInfo", userInfoJson); // 替换原有直接放sysUser的逻辑
//		obj.put("userInfo", sysUser);
...
//	}
	 ...
	 * 根据用户ID获取角色编码（多个逗号分隔）
	 */
	private String getRoleCodeByUserId(String userId) {
		// 1. 查询用户关联的所有角色ID（用户-角色中间表）
		...
		if (userRoleList == null || userRoleList.isEmpty()) {
			return ""; // 无角色返回空字符串，避免前端拿到null
		}

		// 2. 遍历角色ID，查询角色编码并去空拼接
		List<String> roleCodeList = userRoleList.stream()
				.map(sysUserRole -> {
					// 根据角色ID查询角色信息
					SysRole sysRole = sysRoleService.getById(sysUserRole.getRoleId());
					return sysRole != null ? sysRole.getRoleCode() : "";
				})
				.filter(code -> !code.isEmpty()) // 过滤空值
