      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['sectionCode']"
                  :label="get4Label('标段')"
                  name='sectionCode'
                  prop='sectionCode'
                  placeholder="请选择标段"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['businessCode']"
                  :label="get4Label('编号')"
                  name='businessCode'
                  prop='businessCode'
                  placeholder="请选择编号"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['isbn']"
                  :label="get4Label('征订号或书号（ISBN)')"
                  name='isbn'
                  prop='isbn'
                  placeholder="请选择征订号或书号（ISBN)"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['textbookName']"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['publishDate']"
                  :label="get4Label('出版时间')"
                  name='publishDate'
                  prop='publishDate'
                  placeholder="请选择出版时间"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['enableYear']"
                  :label="get4Label('启用学年')"
                  name='enableYear'
                  prop='enableYear'
                  placeholder="请选择启用学年"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('启用学期')"
                 labelWidth="100px"
                 type="list"
                 name='enableSemester'
                 dict="semester"
                v-model="myFormData['enableSemester']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-select
                :label="get4Label('启用状态')"
                 labelWidth="100px"
                 type="list"
                 name='status'
                 dict="use_state"
...
const dataId = ref('')
const backRouteName = ref('TTextbookList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/zbu/tTextbook/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/zbu/tTextbook/edit':'/zbu/tTextbook/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
