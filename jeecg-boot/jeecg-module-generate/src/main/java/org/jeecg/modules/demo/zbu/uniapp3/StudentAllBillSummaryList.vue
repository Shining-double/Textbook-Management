...
<template>
  <PageLayout navTitle="总账单"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './StudentAllBillSummaryData';
defineOptions({
  name: 'StudentAllBillSummaryList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/zbu/studentAllBillSummary/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/zbu/studentAllBillSummary/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
