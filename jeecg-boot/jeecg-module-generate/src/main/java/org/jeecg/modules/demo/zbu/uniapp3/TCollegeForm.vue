      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['collegeName']"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['collegeCode']"
                  :label="get4Label('学院编码')"
                  name='collegeCode'
                  prop='collegeCode'
                  placeholder="请选择学院编码"
                  :rules="[
                                  { required: true, message: '请输入学院编码!'},
                  ]"
                  clearable
              />
        </view>
...
const dataId = ref('')
const backRouteName = ref('TCollegeList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/zbu/tCollege/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/zbu/tCollege/edit':'/zbu/tCollege/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
