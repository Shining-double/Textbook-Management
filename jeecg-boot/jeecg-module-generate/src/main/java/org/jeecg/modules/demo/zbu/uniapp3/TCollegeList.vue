...
<template>
  <PageLayout navTitle="学院表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './TCollegeData';
defineOptions({
  name: 'TCollegeList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/zbu/tCollege/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/zbu/tCollege/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
