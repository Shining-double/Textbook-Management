        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
             <online-popup-link-record
                ...
                labelWidth="100px"
                 name='classId'
                :formSchema="getFormSchema('t_class','id','class_name')"
                v-model:value="myFormData['classId']"
              ></online-popup-link-record>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
             <online-popup-link-record
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('学期')"
                 ...
                 name='semester'
                 dict="semester"
                v-model="myFormData['semester']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-select
                :label="get4Label('生效状态')"
                 labelWidth="100px"
                 type="list"
                 name='selectionStatus'
                 dict="use_state"
                v-model="myFormData['selectionStatus']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
...
const dataId = ref('')
const backRouteName = ref('TTextbookSelectionList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/zbu/tTextbookSelection/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/zbu/tTextbookSelection/edit':'/zbu/tTextbookSelection/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
