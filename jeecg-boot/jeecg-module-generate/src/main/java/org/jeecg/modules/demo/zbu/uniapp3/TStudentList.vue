...
import {columns} from './TStudentData';
defineOptions({
  name: 'TStudentList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/zbu/tStudent/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/zbu/tStudent/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
