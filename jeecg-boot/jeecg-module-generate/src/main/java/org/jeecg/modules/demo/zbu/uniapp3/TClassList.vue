...
<template>
  <PageLayout navTitle="班级表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './TClassData';
defineOptions({
  name: 'TClassList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/zbu/tClass/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/zbu/tClass/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
