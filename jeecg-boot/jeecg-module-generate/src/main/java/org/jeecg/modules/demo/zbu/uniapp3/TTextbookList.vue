...
<template>
  <PageLayout navTitle="教材表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './TTextbookData';
defineOptions({
  name: 'TTextbookList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/zbu/tTextbook/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/zbu/tTextbook/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
