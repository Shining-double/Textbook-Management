...
layout: 'default',
style: {
navigationBarTitleText: '专业表',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="专业表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './TMajorData';
defineOptions({
  name: 'TMajorList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/zbu/tMajor/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/zbu/tMajor/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
