...
public class StudentBillController extends JeecgController<StudentBill, IStudentBillService> {
	...
	private IStudentBillService studentBillService;
	 ...
	 @Autowired
	 private StudentAllBillSummaryController allBillSummaryController;
	  ...
	  * - 专业名称：专业表（TMajor）（通过征订表majorId关联）
...
//	 @RequiresPermissions("zbu:student_bill:sync")
	 ...
	 public Result<String> syncFromSubscription() {
		 try {
			 ...
			 for (TSubscription sub : subscriptionList) {
				 ...
				 bill.setMajorName(majorName);                  // 专业名称（专业表）
			 ...
			 // 6. 批量同步数据
			 if (!billList.isEmpty()) {
				 studentBillService.remove(null); // 清空旧账单
				 studentBillService.saveBatch(billList);
				 // 同步大量数据后，触发全量汇总（确保所有分组都更新）
				 allBillSummaryController.autoSummarySubscriptionData();
				 log.info("同步成功！共{}条记录（领取状态来自领取表）", billList.size());
				 return Result.OK("同步征订数据到个人账单成功！共同步" + billList.size() + "条记录");
			 } else {
	...
	public Result<String> add(@RequestBody StudentBill studentBill) {
		...
		allBillSummaryController.incrementSummary(studentBill, false);
	...
	public Result<String> delete(@RequestParam(name="id",required=true) String id) {
		StudentBill bill = studentBillService.getById(id);
		if (bill != null) {
			studentBillService.removeById(id);
			// 触发实时增量汇总（isDelete=true：删除操作）
			allBillSummaryController.incrementSummary(bill, true);
		}
...
//		this.studentBillService.removeByIds(Arrays.asList(ids.split(",")));
		...
		// 先查询批量删除的账单中，任意一条的分组维度（假设同批次删除的是同一分组，若不是可循环处理）
		if (!idList.isEmpty()) {
			StudentBill firstBill = studentBillService.getById(idList.get(0));
			if (firstBill != null) {
				studentBillService.removeByIds(idList);
				// 触发批量增量汇总
				allBillSummaryController.batchIncrementSummary(
						firstBill.getMajorName(),
						firstBill.getSubscriptionYear(),
						firstBill.getSubscriptionSemester()
				);
			}
		}
    ...
    public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
	 ...
	 public Result<String> batchUpdateReceiveStatus(@RequestBody Map<String, Object> params) {
		 try {
			 ...
			 // 6. 返回结果
			 if (updateSuccess) {
				 // 批量修改后，触发汇总（取第一条账单的分组维度）
				 if (!ids.isEmpty()) {
					 StudentBill firstBill = studentBillService.getById(ids.get(0));
					 if (firstBill != null) {
						 allBillSummaryController.incrementSummary(firstBill, false);
					 }
				 }
