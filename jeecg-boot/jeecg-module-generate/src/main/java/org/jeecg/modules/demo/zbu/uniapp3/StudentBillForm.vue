        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
             <online-popup-link-record
                ...
                labelWidth="100px"
                 name='studentId'
                :formSchema="getFormSchema('t_student','id','student_id')"
                v-model:value="myFormData['studentId']"
              ></online-popup-link-record>
        </view>
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
             <online-popup-link-record
                :label="get4Label('教材名称')"
                labelWidth="100px"
                 name='textbookName'
                :formSchema="getFormSchema('t_textbook','id','textbook_name')"
                v-model:value="myFormData['textbookName']"
              ></online-popup-link-record>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
             <online-popup-link-record
                :label="get4Label('教材定价')"
                labelWidth="100px"
                 name='price'
                :formSchema="getFormSchema('t_textbook','id','price')"
                v-model:value="myFormData['price']"
              ></online-popup-link-record>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['discountPrice']"
                   :label="get4Label('教材费用')"
                   name='discountPrice'
                   prop='discountPrice'
                   placeholder="请选择教材费用"
                   inputMode="numeric"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
             <online-popup-link-record
                :label="get4Label('征订状态')"
                labelWidth="100px"
                 name='subscribeStatus'
                :formSchema="getFormSchema('t_subscription','id','subscribe_status')"
                v-model:value="myFormData['subscribeStatus']"
              ></online-popup-link-record>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
             <online-popup-link-record
                :label="get4Label('领取状态')"
                labelWidth="100px"
                 name='receiveStatus'
                :formSchema="getFormSchema('t_receive','id','receive_status')"
                v-model:value="myFormData['receiveStatus']"
              ></online-popup-link-record>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
...
const dataId = ref('')
const backRouteName = ref('StudentBillList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/zbu/studentBill/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/zbu/studentBill/edit':'/zbu/studentBill/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
