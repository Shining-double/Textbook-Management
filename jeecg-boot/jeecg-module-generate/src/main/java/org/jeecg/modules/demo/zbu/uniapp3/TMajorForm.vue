      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['majorCode']"
                  :label="get4Label('专业编码')"
                  name='majorCode'
                  prop='majorCode'
                  placeholder="请选择专业编码"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('TMajorList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/zbu/tMajor/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/zbu/tMajor/edit':'/zbu/tMajor/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
