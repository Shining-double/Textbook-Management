...
<template>
  <PageLayout navTitle="教材选用表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './TTextbookSelectionData';
defineOptions({
  name: 'TTextbookSelectionList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/zbu/tTextbookSelection/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/zbu/tTextbookSelection/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
