      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['studentName']"
                  :label="get4Label('学生姓名')"
                  name='studentName'
                  prop='studentName'
                  placeholder="请选择学生姓名"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['admissionYear']"
                  :label="get4Label('入学年份')"
                  name='admissionYear'
                  prop='admissionYear'
                  placeholder="请选择入学年份"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('TStudentList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/zbu/tStudent/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/zbu/tStudent/edit':'/zbu/tStudent/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
