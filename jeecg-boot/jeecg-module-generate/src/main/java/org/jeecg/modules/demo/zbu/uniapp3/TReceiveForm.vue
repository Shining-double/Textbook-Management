        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
             <online-popup-link-record
                ...
                labelWidth="100px"
                 name='subscriptionId'
                :formSchema="getFormSchema('t_subscription','id','textbook_id')"
                v-model:value="myFormData['subscriptionId']"
              ></online-popup-link-record>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('领取状态')"
                 ...
                 name='receiveStatus'
                 dict="receive_status"
                v-model="myFormData['receiveStatus']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['receiveRemark']"
                  :label="get4Label('领取备注')"
                  name='receiveRemark'
                  prop='receiveRemark'
                  placeholder="请选择领取备注"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('TReceiveList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/zbu/tReceive/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/zbu/tReceive/edit':'/zbu/tReceive/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
