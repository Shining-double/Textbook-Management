...
<template>
  <PageLayout navTitle="征订表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './TSubscriptionData';
defineOptions({
  name: 'TSubscriptionList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/zbu/tSubscription/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/zbu/tSubscription/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
